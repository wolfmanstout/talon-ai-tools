...
- **Linting**: Run `ruff format`, then `ruff check` without `--fix`. Apply relevant lint fixes manually and only to lines changed for the task. Finally, run `prek run --files` followed by a list of changed files.
